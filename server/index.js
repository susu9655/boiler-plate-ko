  ...
  app.get('/', (req, res) => {
    res.send('Hello World!qmfkgfd.,dsffm')
  })
  ...
  app.post('/api/users/login',(req,res)=>{
    ...
    User.findOne({email: req.body.email }, (err,user)=>{
      ...
      //데이터베이스에서 요청한 이메일이 있다면 비밀번호가 맞는지 확인
      user.comparePassword(req.body.password,(err,isMatch)=>{
        if(!isMatch)
        return res.json({loginSuccess:false,message:"비밀번호가 트렸습니다."})

        //비밀번호까지 맞다면 유저를 위한 Token생성
        user.generateToken((err,usr)=>{
          if(err)return res.status(400).send(err)

          //토큰을 쿠키, 로컬스토리지 등등에 저장 가능
